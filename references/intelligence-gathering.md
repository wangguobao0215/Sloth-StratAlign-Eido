...
> 适用于: Module 4 (Q4.1) — 这是数据质量风险最高的环节。
...
> 适用于: Module 1 (Q1.4 财务目标), Module 5 (Q5.1 预算) — 辅助预填。
...
| 问题 | 必须搜索 | 搜索词模板 | 预期输出 |
|---|---|---|---|
| Q4.1 IT系统清单 | **强制** | **执行完整5阶段工作流 (§2.2)** | 结构化系统清单 [C1~C4] |
| Q4.2 集成现状 | 建议 | `"{全称}" 数据中台 OR ESB OR 集成平台` | 集成架构线索 [C3] |
| Q4.4 云化程度 | 建议 | `"{全称}" 上云 OR 云计算 OR 阿里云 OR 华为云` | 云化策略 [C2/C3] |

### Module 5: 投资组合

| 问题 | 建议搜索 | 搜索词模板 | 预期输出 |
|---|---|---|---|
| Q5.1 总体预算 | 可选 | `"{全称}" IT预算 OR 信息化投入 OR 数字化投资` | 投资规模线索 [C2/C3] |
